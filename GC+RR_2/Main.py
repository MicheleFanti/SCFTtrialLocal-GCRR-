...
def chempot_per_resclass(s, sequence, steps_per_residue, rhoP, u_ev_k, vchi_pp, lambdas, charges, h_as, V_kernel_k):
    ...
    for a_key in charges:
        # charges[a_key] is an array of charges for each residue type in the sequence order
...
def main(sequence, rhop0, gridshape, box_lengths, b2_over_6, dx, u_vectors, ang_weights, epsilon_hb, vchi_pp, alpha_pb, relax_eta, relax_mu):
    ...
    def make_chempot_fields(eta_field, mu_dict, current_rho_class, current_alpha_pb, current_vchi_pp, current_epsilon_hb, h_as_fields, V_k_kernel, K_ev_kernel, K_hb_kernel, K_pb_kernel, hb_kernel_ang_param, ang_pb_param):
        ...
        for terminal_type in ['Nsc', 'Csc']:
            ...
            w_sidechains.append(w + eta_field[..., None]*np.ones((Nx, Ny, Nz, Nang)) + mu_dict.get(terminal_type, 0.0)[..., None]*np.ones((Nx, Ny, Nz, Nang))) # Add eta and mu contributions

        def w_backbone(s):
            # This function closes over the current iteration's fields
            return chempot_per_resclass(s, sequence, steps_per_residue, rhoP, K_ev_kernel, current_vchi_pp,
                                  lambdas, charges, h_as_fields, V_k_kernel) + eta_field + mu_dict[sequence[int(s/steps_per_residue)]]
    ...
    for it in range(max_iter):
        ...
        total_current_rho = 0
        diff =0
        eta, mu = update_eta_mu(rho_class, rho0_per_class, eta, mu, dV, ang_weights,relax_eta, relax_mu)
        for c_key in rho0_per_class:
            if c_key == 'Nsc':
